...
- [ ] T001 `Options/Onboarding.cs`: ölü `ActivationBaseUrl` → `PublicBaseUrl` (Required) +
...
- [ ] T005 [US1] Hosted form sayfaları: `GET /onboarding/form/{token}` (029 alan seti, type'a
...
- [ ] T007 [P] [US2] Domain testleri ÖNCE (red): `CredentialRevealLink.Create/Consume/IsUsable/Kill`
  — `tests/Merchant.Api.Tests/CredentialRevealLinkTests.cs`
- [ ] T008 [US2] `CredentialRevealLink` aggregate (green) —
  `src/services/Merchant.Api/Domains/CredentialRevealLinks/CredentialRevealLink.cs`;
  Marten index `Token` + `MerchantId` (Program.cs)
- [ ] T009 [US2] Approve kancası: `AdminApproveRegistration` handler'ına reveal link üretimi +
...
- [ ] T010 [US2] Teslim sayfası: `GET /onboarding/reveal/{token}` — usable ise MerchantId +
...
- [ ] T011 [US2] `admin_resend_credential_link` MCP tool (merchant.admin): Approved merchant'ın
...
- [ ] T013 [P] [US3] `GetOnboardingApplicationStatus` query + `GET /api/v1/onboarding/
...
- [ ] T014 [P] [US3] `ValidateMerchantCredentials` query + `POST /api/v1/onboarding/credentials/
...
- [ ] T018 [P] Merchant `FLOW.md` güncelle (İLKE VII, aynı PR): başvuru doğuşu hosted form; teslim
  mail + tek gösterimlik sayfa; eski MCP teslim adımı sil — `scripts/check-flow-links.sh` yeşil.
- [ ] T019 [P] `CLAUDE.md` Merchant BC satırı güncelle (hosted onboarding + teslim; 029 MCP teslim
  yolu emekli).
- [ ] T020 Tam doğrulama: `dotnet build` + `dotnet test` + guard script'leri.
