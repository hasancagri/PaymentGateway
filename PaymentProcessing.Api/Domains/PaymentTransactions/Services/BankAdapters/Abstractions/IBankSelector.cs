using PaymentProcessing.Api.Domains.PaymentTransactions.ValueObjects;
        ...
        Guid merchantId, string currency, CardProfile cardProfile, CancellationToken ct)
    {
        ...
        var bankCommissions = await session.Query<BankCommissionSummary>()
            .Where(bc => eligibleBankIds.Contains(bc.BankId)
                      && bc.CardBrand == cardProfile.CardBrand
                      && bc.CardType == cardProfile.CardType
                      && bc.TransactionRegion == cardProfile.TransactionRegion)
            .ToListAsync(token: ct);
