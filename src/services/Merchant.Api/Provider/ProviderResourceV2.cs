...
public class ProviderResourceV2
{
    ...
    protected static Dictionary<string, string> GetHttpHeadersWithRequestBody(BaseRequestV2 request, String url, ProviderOptions options)
    {
    ...
    protected static Dictionary<string, string> GetHttpHeadersWithUrlParams(BaseRequestV2 request, String url, ProviderOptions options)
    {
        Dictionary<string, string> headers = GetCommonHttpHeaders(request, url, options);
    ...
    private static Dictionary<string, string> GetCommonHttpHeaders(BaseRequestV2 request, String url, ProviderOptions options)
    {
        Dictionary<string, string> headers = new Dictionary<string, string>();
        headers.Add("Accept", "application/json");
        headers.Add(CLIENT_VERSION_HEADER_NAME, ProviderConstants.CLIENT_VERSION);
        headers.Add(CONVERSATION_ID_HEADER_NAME, request.ConversationId);
        return headers;
    }

    private static String PrepareAuthorizationStringWithRequestBody(BaseRequestV2 request, String url, ProviderOptions options)
    {
    ...
    private static String PrepareAuthorizationStringWithUrlParam(BaseRequestV2 request, String url, ProviderOptions options)
    {
        String randomKey = GenerateRandomKey();
        String uriPath = FindUriPath(url);
