...
{
    Task<ApiResult<IdResult>> CreateAsync(CreateMerchantRequest request, CancellationToken ct = default);
...
public class MerchantApiClient : ApiClientBase, IMerchantApiClient
{
    ...
    public Task<ApiResult<IdResult>> CreateAsync(CreateMerchantRequest request, CancellationToken ct = default) =>
        SendAsync<IdResult>(() => Http.PostAsJsonAsync("/api/v1/merchants", request, ct), ct);
