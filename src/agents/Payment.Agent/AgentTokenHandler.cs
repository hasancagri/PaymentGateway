...
// 011: Payment.Agent makine kimliği (SagaTokenHandler deseni — D7). MCP transport'un HttpClient'ına
// takılır; client_credentials token'ı static cache'lenir, süresine 30 sn kala yenilenir (süre
// dolmuşsa credential'la anında taze alınır). Token edinilemezse istisna yüzeye çıkar (sessiz
// başarı yok) — tool çağrısı anlaşılır hata döner.
public sealed class AgentTokenHandler(IConfiguration config) : DelegatingHandler
{
    private const string Scopes = "payment.read payment.write";
