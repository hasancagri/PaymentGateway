...
{
    private record DefineMerchantCommissionRequest(
        Guid MerchantId,
    ...
    {
        app.MapGet("/merchant-commissions", async ([AsParameters] Guid merchantId, [FromQuery] int page,
            CommissionApiClient client, CancellationToken ct) =>
        {
        ...
        app.MapGet("/merchant-commissions/{id:guid}",
            async (Guid id, CommissionApiClient client, CancellationToken ct) =>
            {
        ...
        app.MapPost("/merchant-commissions",
            async (DefineMerchantCommissionRequest req, CommissionApiClient client, CancellationToken ct) =>
            {
        ...
        app.MapPatch("/merchant-commissions/{id:guid}/rate", async (Guid id, UpdateCommissionRateRequest req,
            CommissionApiClient client, CancellationToken ct) =>
        {
