...
namespace MerchantManagement.Api.Modules.MerchantManagement.Merchants.Features.Queries;
...
public static class GetMerchantById
{
    ...
    public class GetMerchantByIdHandler
    {
        ...
        {
            var merchant = await db.Set<Merchant>().FirstOrDefaultAsync(x => x.Id == query.MerchantId, ct);
            if (merchant is null)
