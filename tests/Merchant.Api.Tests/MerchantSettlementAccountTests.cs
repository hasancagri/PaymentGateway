using Merchant.Api.Domains.MerchantSettlementAccounts;
using Xunit;

namespace Merchant.Api.Tests;

public class MerchantSettlementAccountTests
{
    ...
    private static ResultDomain<MerchantSettlementAccount> CreateValid() =>
        MerchantSettlementAccount.Create(ValidMerchantId, ValidBankCode, ValidIban, ValidOwner, "123", "TL hesabı");
    ...
    {
        var result = MerchantSettlementAccount.Create(
    ...
    public void Create_bozuk_mod97_IBAN_Error_INVALID_FORMAT()
    {
        ...
        Assert.Contains(result.Messages!, m =>
            m.Property == nameof(MerchantSettlementAccount.Iban) &&
    ...
    public void Create_bos_merchantId_Error_required()
    {
        ...
        Assert.False(result.IsSuccess);
        Assert.Contains(result.Messages!, m =>
            m.Property == nameof(MerchantSettlementAccount.MerchantId) &&
    ...
    public void Create_bos_bankCode_Error_required(string bankCode)
    {
        ...
        Assert.False(result.IsSuccess);
        Assert.Contains(result.Messages!, m =>
            m.Property == nameof(MerchantSettlementAccount.BankCode) &&
            m.Code == CommonResourceConstants.COMMON_MESSAGE_VALUE_IS_REQUIRED);
    }
    ...
    public void Create_bos_ownerName_Error_required()
    {
        ...
        Assert.False(result.IsSuccess);
        Assert.Contains(result.Messages!, m =>
            m.Property == nameof(MerchantSettlementAccount.AccountOwnerName) &&
            m.Code == CommonResourceConstants.COMMON_MESSAGE_VALUE_IS_REQUIRED);
    }
